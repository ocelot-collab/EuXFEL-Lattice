"""Cathode to SASE3, driven by a setpoints file.

The same run as ``s2e_up_to_SA3.py``, with the RF preamble and the r1/r2/r3
...
Run it from inside ``s2e_scripts/`` -- ``data_dir`` is relative::
    ...
    cd s2e_scripts && python s2e_up_to_SA3_volts.py
