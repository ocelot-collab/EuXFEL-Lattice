...
lat = MagneticLattice(i1.cell + l1.cell  + b1d.cell)
...
# B1D
df_b1d = df[df["SECTION"] == "B1D"]

# concatenate sections
s = np.concatenate((df_i1["S"].to_numpy() , df_l1["S"].to_numpy(),df_b1["S"].to_numpy(), df_b1d["S"].to_numpy()))+23.2
beta_x = np.concatenate((df_i1["BETX"].to_numpy() , df_l1["BETX"].to_numpy(),df_b1["BETX"].to_numpy(),  df_b1d["BETX"].to_numpy()))
beta_y = np.concatenate((df_i1["BETY"].to_numpy() , df_l1["BETY"].to_numpy(),df_b1["BETY"].to_numpy(),  df_b1d["BETY"].to_numpy()))
D_x = np.concatenate((df_i1["DX"].to_numpy() , df_l1["DX"].to_numpy(),df_b1["DX"].to_numpy(),  df_b1d["DX"].to_numpy()))
D_y = np.concatenate((df_i1["DY"].to_numpy() , df_l1["DY"].to_numpy(),df_b1["DY"].to_numpy(),  df_b1d["DY"].to_numpy()))
